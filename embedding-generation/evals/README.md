...
regression gate (`./run-question-eval.sh`); use the golden set to measure
broader capability and the hive-mind set to measure the requested acceptance
area. Measured results for each change live in `results/`; a change that moves
the regression gate must list the questions it moves.
